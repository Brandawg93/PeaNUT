  ...
  const editInput = (key: string, value: string) => (
    <div className='flex'>
      <input
        ...
        className='w-full grow rounded border border-gray-300 bg-transparent pl-2 text-gray-800 dark:border-gray-800 dark:text-gray-100'
        defaultValue={value}
      />
      <div className='flex'>
            ...
            {useTreeData && (
              <div className='flex h-[28px] flex-col justify-center'>
                {table.getIsAllRowsExpanded() ? (
                  <HiOutlineChevronDown className='text-primary h-4 w-4' />
                ) : (
                  <HiOutlineChevronRight className='text-primary h-4 w-4' />
                )}
              </div>
            )}
