...
const GaugeLabel = ({
  ...
  return (
    <g>
      {typeof warningAt === 'number' && !Number.isNaN(warningAt) && warningAt > 0 && (
        <Marker cx={cx} cy={cy} pct={warningAt} color='#f59e0b' label={`${warningAt}%`} />
      )}
      {typeof lowAt === 'number' && !Number.isNaN(lowAt) && lowAt > 0 && (
        <Marker cx={cx} cy={cy} pct={lowAt} color='#ef4444' label={`${lowAt}%`} />
      )}
